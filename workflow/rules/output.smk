...
def get_final_output():
    ...
    final_output.extend(
        expand(
            "results/intermediate/{ID}/table/transcriptanno_{SAMPLE}_WPS_normalized.{COV}x.{GENOME}.tsv.gz",
            zip,
            SAMPLE=samples["sample"],
            COV=config["coverage"],
            ID=samples["ID"],
    ...
    final_output.extend(
        expand("results/intermediate/{ID}/FFT_table/transcriptanno-{SAMPLE}-FFT_table.{COV}x.{GENOME}.tsv.gz",
            zip,
            SAMPLE=samples["sample"],
            COV=config["coverage"],
            ID=samples["ID"],
    ...
    final_output.extend(
        expand("results/intermediate/{ID}/table/transcriptanno_{SAMPLE}_COV_normalized.{COV}x.{GENOME}.tsv.gz",
            zip,
            SAMPLE=samples["sample"],
            COV=config["coverage"],
            ID=samples["ID"],
